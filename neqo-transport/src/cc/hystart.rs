...
pub enum HyStartCssBaseline {
    ...
    /// though they would still have triggered CSS entry.
    #[default]
    CurrentRoundMinRtt,
    /// Baseline = `lastRoundMinRTT + rttThresh` (the CSS entry threshold).
    ///
    /// CSS is only exited when the RTT drops below the level that would have triggered
    /// CSS entry, avoiding oscillation between CSS entry and spurious exit.
    EntryThreshold,
}
